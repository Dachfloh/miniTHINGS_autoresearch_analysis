...
fold, and prints the minimum validation loss and its epoch.
...
def epoch_losses(model) -> tuple[list[int], list[float], list[float]]:
    ...
    batches (matching train_model's own epoch print), and the val loss is
    taken at the last time step or averaged depending on the run's config.
    """
    for attr in ("val_losses", "loss"):
...
def plot_loss_curves(run_path: str | Path, out_path: str | Path | None = None) -> None:
    ...
    for fold_idx, (ax, pt) in enumerate(zip(axes[:, 0], pt_files)):
        model = load_model(pt)
        epochs, train_loss, val_loss = epoch_losses(model)

        min_val_loss = min(val_loss)
        min_val_ep = epochs[val_loss.index(min_val_loss)]  # first min
        suffix = f" (fold {fold_idx})" if len(pt_files) > 1 else ""
        label = f"fold {fold_idx}: " if len(pt_files) > 1 else ""
        print(f"{label}minimum val loss {min_val_loss:.6f} at epoch {min_val_ep}")
